...
{
    public partial class BattleDecrementAttackerRelEffectsSystem : BaseEcsSystem
    {       
        private readonly EcsPoolInject<EffectInstanceInfo> pool = default;        
        private readonly EcsPoolInject<AttackerRef> subjectPool = default;
        ...
        public override void RunIfActive(IEcsSystems systems)
        {
            foreach (var entity in filter.Value)
            {
                ...
                while (en.MoveNext())
                {
                    ...
                    if (effect.Rule.Key.RelationsEffectType switch
                    {
                        RelationsEffectType.AlgoRevenge => effect.StartTurn < turnInfo.Turn,
                        RelationsEffectType.AlgoTarget => effect.StartTurn < turnInfo.Turn,
                        _ => effect.StartTurn < turnInfo.Turn
                    })
                        effect.UsageLeft--;                    
                }
            }
